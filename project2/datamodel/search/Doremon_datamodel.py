...
@pcc_set
class DoremonLink(Link):
    USERAGENTSTRING = "Doremon"
...
@subset(DoremonLink)
class DoremonUnprocessedLink(object):
    @staticmethod
    def __predicate__(l):
...
@impure
@subset(DoremonUnprocessedLink)
class OneDoremonUnProcessedLink(DoremonLink):
    __limit__ = 1
    @staticmethod
    def __predicate__(l):
        return not (l.download_complete or l.error_reason)

@projection(DoremonLink, DoremonLink.url, DoremonLink.download_complete)
class DoremonProjectionLink(object):
    pass

@trigger(OneDoremonUnProcessedLink, TriggerTime.after, TriggerAction.read)
...
@trigger(DoremonLink, TriggerTime.after, TriggerAction.update)
