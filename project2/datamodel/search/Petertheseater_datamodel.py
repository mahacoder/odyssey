...
@pcc_set
class PetertheseaterLink(Link):
    USERAGENTSTRING = "Petertheseater"
...
@subset(PetertheseaterLink)
class PetertheseaterUnprocessedLink(object):
    @staticmethod
    def __predicate__(l):
...
@impure
@subset(PetertheseaterUnprocessedLink)
class OnePetertheseaterUnProcessedLink(PetertheseaterLink):
    __limit__ = 1
    @staticmethod
    def __predicate__(l):
        return not (l.download_complete or l.error_reason)

@projection(PetertheseaterLink, PetertheseaterLink.url, PetertheseaterLink.download_complete)
class PetertheseaterProjectionLink(object):
    pass

@trigger(OnePetertheseaterUnProcessedLink, TriggerTime.after, TriggerAction.read)
...
@trigger(PetertheseaterLink, TriggerTime.after, TriggerAction.update)
