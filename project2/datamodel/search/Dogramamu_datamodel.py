...
@pcc_set
class DogramamuLink(Link):
    USERAGENTSTRING = "Dogramamu"
...
@subset(DogramamuLink)
class DogramamuUnprocessedLink(object):
    @staticmethod
    def __predicate__(l):
...
@impure
@subset(DogramamuUnprocessedLink)
class OneDogramamuUnProcessedLink(DogramamuLink):
    __limit__ = 1
    @staticmethod
    def __predicate__(l):
        return not (l.download_complete or l.error_reason)

@projection(DogramamuLink, DogramamuLink.url, DogramamuLink.download_complete)
class DogramamuProjectionLink(object):
    pass

@trigger(OneDogramamuUnProcessedLink, TriggerTime.after, TriggerAction.read)
...
@trigger(DogramamuLink, TriggerTime.after, TriggerAction.update)
